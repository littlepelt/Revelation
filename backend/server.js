...
// Раздача загруженных обложек и текстов
app.use('/api/uploads/covers', express.static(path.join(__dirname, 'books', '_uploads', 'covers')));
app.use('/api/uploads/texts', express.static(path.join(__dirname, 'books', '_uploads', 'texts')));
...
app.get('/api/books/reviews/latest', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT r.id, r.rating, r.comment, r.created_at, r.likes, r.dislikes, r.book_id,
             u.id as user_id, u.username, u.avatar_url,
...
app.get('/api/books/tag/:tag', async (req, res) => {
  ...
  const tagMapping = {
    ...
    'strategy': 'Стратегия',
    'tactics': 'Тактика',
    'combat_training': 'Боевая подготовка',
    'phys_training': 'Физ. подготовка',
